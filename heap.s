...
_ralloc	
		...
		MOV		R9, #16
		MUL		R7, R3, R9				;[R7 = Act_Entire_Size] = Entire * 16
		
		MUL		R8, R4, R9				;[R8 = Act_Half_Size] = Half * 16 
...
_split_parent	;Parent memory must be split
		...
		STRH	R8, [R5]				; Store act_half_size into mcb[midpoint]
		B		_ralloc_return	
...
_ralloc_return
		MOV		R0, R6					; Move the heap address into r0
		POP     {R4-R11, LR}			; Restore registers
		MOV		PC, LR					; Return
...
_rfree
        ...
        LDR     R1, =MCB_BOT
        CMP     R6, R1                  ; Check if buddy is beyond MCB_BOT
        BHS     _rfree_done
...
_rfree_right
        ; Right block case
        LDR     R1, =MCB_TOP
        SUBS    R6, R0, R6              ; R6 = buddy_addr (mcb_addr - mcb_chunk)
