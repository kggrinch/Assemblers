...
void *_ralloc( int size, int left, int right ) {
  ...
  int entire = right - left  + mcb_ent_sz;  // The entires range the mcb will cover # of entries
  int half = entire / 2;                    // The half range the mcb will cover # of entires
  int midpoint = left + half;               // Current midpoint of mcb
  int heap_addr = NULL;                     // address unkown at first
  int act_entire_size = entire * 16;        // Total bytes within the entries
  int act_half_size = half * 16;            // half of the total bytes within the entires
